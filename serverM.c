...
// Including the required set of header files which is essential for the execution of the program.
#include <sys/wait.h>
...
// Declaring some global amcros that will be used throughout the execution of the program.
...
#define ADDR_OF_MIRROR_IP "127.0.0.1"
#define STATUS_OF_DEBUG_MODE 0
#define SERVER_CONNECTION_TYPE 'S'
#define NUM_OF_ENTRY_PORT 56789
...
#define MIRROR_CONNECTION_TYPE 'M'
...
#else // Setting disabled mode for debugging.
...
// Balancing load between Main and Mirror servers.
char msLoadBalancerBtwClientAndMirror(int msClientNumber);
...
// Declared function for client to perform data transfer.
...
// Declared function for redirection client connection to mirror connection.
int msRedirectionFromClientToMirror(int msFileDesOfClientSock);
...
// Declared function for the conversion to unix time from input time.
...
Function Purpose - Logic execution of the main function for Cient-Mirror-Server Connection.
...
int main(int msArgsCount, char *msArgsValue[])
{
	...
	msServerAddress.sin_port = htons(NUM_OF_ENTRY_PORT);
	...
	{	
		...
		// Checking to which server, it is suppose to be connected either from primary server or mirror.
		char msCheckConnectionType = msLoadBalancerBtwClientAndMirror(msClientNum);
		
		// Connecting to the Main/Primary Server.
		if(msCheckConnectionType == SERVER_CONNECTION_TYPE){
		...
		else if(msCheckConnectionType == MIRROR_CONNECTION_TYPE) {
			msClientSockFileDesc = accept(msServerSockFileDesc, (struct sockaddr *) NULL, NULL);
			printf("Server reached its limit. So, requests redirected to client-> %d\n", msClientNum);
			
			// Now, child process will be forked for redirecting client to mirror.
			if (!fork()){	
				msRedirectionFromClientToMirror(msClientSockFileDesc);
				close(msClientSockFileDesc);
				exit(0);
			}
			waitpid(0, &msStatusOfChildProcess, WNOHANG);
...
void msClientProcess(int msFileDesOfClientSock)
{
	...
	msResponseForSendingText(msFileDesOfClientSock, "Now, you can send commands.");
	...
	{	
		...
		{		
			printf("Request from client: QUIT.\n");
...
/*
Function Name - msLoadBalancerBtwClientAndMirror

Function Purpose - 1. Determines connection type - Eother server or mirror.

2. If msClientNumber value is in range (1,6) then it will be a server connection.
   else if msClientNumber value is in range (7,12) then it will be a mirror connection.
   Then it will toggle connection between server(13,15,17,19,...) to miror(14,16,18,20,...) when new connection request comes in.
   
3. It depicts the concept of Load Balances of connection requests from client to server and mirror.
*/
char msLoadBalancerBtwClientAndMirror(int msClientNumber) {
    if (msClientNumber >= 1 && msClientNumber <= 6) {
        return SERVER_CONNECTION_TYPE;
    } else if (msClientNumber >= 7 && msClientNumber <= 12) {
        return MIRROR_CONNECTION_TYPE;
    } else {
        static char msResult = MIRROR_CONNECTION_TYPE;  
        msResult = (msResult == SERVER_CONNECTION_TYPE) ? MIRROR_CONNECTION_TYPE : SERVER_CONNECTION_TYPE;  
        return msResult;
    }
}

/*
Function Name - msRedirectionFromClientToMirror

Function Purpose - 1. This function mainly handles the redirection of client's connection  from a server to a mirror.

2. It will send a signal to client which indicates redirection, by providing IP address and Port Number of mirror's connection and confirms the successful redirection.

3. The main purpose is to establish a successful connection with Mirror instead of making a connection to server each time.
*/

int msRedirectionFromClientToMirror(int msFileDesOfClientSock){
	msAddressInformation msAddrInfo;
	
	// Sending response to Mirror Connection instead of Server.
	long msRespType = RESP_TYPE_STRUCT;
	send(msFileDesOfClientSock, &msRespType, sizeof(msRespType), 0);
	
	// Sending IP Adress along with port number for mirror connection.
	strcpy(msAddrInfo.msIPAddr, ADDR_OF_MIRROR_IP);
	msAddrInfo.msPortNum = NUM_OF_MIRROR_PORT;
	send(msFileDesOfClientSock, &msAddrInfo, sizeof(msAddressInformation), 0);
	printf("The redirection of Client %d - success.\n", msClientNum);
    ...
    if (msPipe != NULL) { // Piping Operation
        ...
        if (fgets(msReadLine, sizeof(msReadLine), msPipe) != NULL) { // Getting the first line of the pipe output to read.
            ...
            if (stat(msReadLine, &msFileInfo) == 0) { // Utilizing stat() function to obtain file information.
                ...
                sprintf(msResponse,"Fails to get file details for %s", msReadLine);
            }
        } else {
...
{	
    ...
    DEBUG_LOG("Path of Home directory is -> %s\n", msHomeDir);
    DEBUG_LOG("%d The total number of parsed file Names as ->", msFileTotalCount);
